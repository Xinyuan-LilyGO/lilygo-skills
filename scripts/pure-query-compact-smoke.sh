...
if [[ "${1:-}" != "--dry-run" ]]; then
  echo "pure-query-compact-smoke requires --dry-run" >&2
  exit 2
fi
...
const capsule = plan.context_capsule;
...
check("lookup has source-query actions", actions.some((action) => action.id === "source-query-i2c"), actions);
...
check("lookup has no recipes", (plan.recipe_ids || []).length === 0, plan.recipe_ids);
