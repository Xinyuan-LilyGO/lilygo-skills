...
产品是一个很小的 **meta Skill**(`skills/lilygo-router/SKILL.md`)加一个薄 **Node CLI**
(`bin/` 下的 JS context kernel)。`SKILL.md` 是*操作系统*:查询协议、调试循环、诚实
规则,全部以散文形式承载。CLI 是确定性的 **Context Kernel**:判断 prompt 是否是 LilyGO
工作、涉及哪块板子、哪些带来源的事实可以安全注入,然后把一个紧凑胶囊和精确的
`source query` 命令交给 AI,让它按需拉取更深的内容。认板与事实选择都是数据驱动(只读
`data/**`,绝不内联引脚);“这个引脚我能不能说”由诚实规则和 source model 强制。内核以
`.mjs` 发布,跑在每个宿主本就有的 Node 上,没有需要编译的编译器或二进制。
...
命令面刻意保持小而稳定。命令名是 `lilygo-skills`(安装的 shim,exec
`node <root>/bin/lilygo-skills.mjs`):
...
lilygo-skills verify sources --board <board-id> [--topic <t>] --json   # 在线复证(OK / DRIFT / UNREACHABLE)
lilygo-skills doctor --json                                            # 数据完整性自测
lilygo-skills hook <claude|codex>                                      # 推送厚板级胶囊(stdin:{"prompt":..})
```

`context` 是一击入口:它从项目自动认板(先读 `.lilygo-skills/project.json`,否则嗅探
`platformio.ini`、`sdkconfig`、`*.ino`),返回匹配的 skill id、top 事实、验证等级和后续
查询命令。`hook` 是推送边界,Claude Code 的 hook 调它在模型作答前内联板子的关键引脚。

操作深度——调试循环、框架 setup、LVGL/LoRa/电源排错、诚实规则——以散文形式活在
`SKILL.md` 及其 guides 里,不是额外子命令。板级事实 ingest 与来源复证由 Node 数据管线
(`pipeline/**`、`eval/**`)在作者时处理,不属于日常 CLI。

## 数据驱动的薄 reader

认板与事实选择读 `data/**` 下的提交 JSON——板子嗅探规则(`data/sniff-rules.json`)、
prompt 关键词(`data/facts/prompt-keywords.json`)、topic 字段
(`data/facts/topic-fields.json`)与带来源的 fact pack(`data/facts/board-fact-packs.json`)。
reader 绝不内联引脚:它返回的每个值都来自带官方 URL、行范围与 sha256 的 fact pack。加一块
板或加深一块板,是走 ingest 管线改数据,不是改代码。
...
**Node.js** 是唯一前置。安装器把 Node dispatcher(`bin/**`)与数据模型(`data/**`)作为
...
追加一段带标记的小节。若 `settings.json` 不是合法 JSON,安装器会明确报错并打印手工片段,
不碰这个文件。

宿主工具链(Arduino CLI、PlatformIO、ESP-IDF、esp-rs、board core、串口工具、
LoRa/GNSS 库)保持显式——报告安装提示,只通过用户授权步骤运行,绝不隐式安装。
...
```bash
npm test                                            # 单测 + CLI 契约 + hook 值对齐 + CJK 路由
npx tsc -p tsconfig.json --noEmit                   # 严格 typecheck JS 内核
...
bash scripts/ci-gate.sh                             # 聚合确定性门禁
```

`coverage-gate.js` 针对每个 eval prompt 直接给模型*真实收到的注入胶囊*按期望事实打分
(基线只上不下)。因此裁剪脚手架永远无法悄悄让模型实际看到的事实退化。`ci-gate.sh`
聚合 JS 核心门(`npm test`、typecheck、`doctor`、在线 `verify sources`)、
data/pipeline/provenance 校验、板级三问测试、scorecard grading,以及 install → hook
集成 smoke。
