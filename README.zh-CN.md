...
命令；hook 接线损坏仍是 failure。它不宣称硬件、OTA、串口、RF、显示或传感器成功。
...
证据和 context digest；不保存串口、Wi-Fi、OTA endpoint、token、原始日志或私有
evidence path，也不会替代官方 source facts。用户要求重新运行或重新验证时，runtime
